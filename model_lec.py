...
from typing import (
    cast,Any,Optional,Union,Iterator,
    Iterable,Counter,Callable,Protocol,TypeDict,List,overload, Tuple
)
import weakref
from abc import *
import random
from model_lec import TestingKnownSample, TrainingKnownSample
...
class SamplePartition(List[SampleDict], abc.ABC):
    ...
    @overload
    def __init__(self,iterable: Optional[Iterable[SampleDict]] = None,*, training_subset: float = 0.88,)-> None:
        ...
    ...
    @abc.abstractproperty
    @property
    def test(self)-> list[TestingKnownSample]:
        ...


class ShuffleingSamplePartition(SamplePartition):
    def __init__(
        self, 
        iterable: Optional[Iterable[SampleDict]]= None,
        *,
    ...
    def shuffle(self):
        if not self.split:
            random.shuffle(self)
            self.split = len(self) * self.training_subset
...
class DealingPartition(abc.ABC):
    def __init__(
        ...
        *,
        trainig_subset: Tuple[int,int] = ( 8 , 10)
    ) -> None:
        ...
    @abc.abstractproperty
    @property
...
class CountingDealingPartition(DealingPartition):
    def __init__(
        ...
        self.trainig_subset = trainig_subset
        self.counter = 0
        self._trainig: list[TrainingKnownSample] = []
        self._testint: list[TestingKnownSample] = []
        if items:
            self.extend(items)
    ...
    def append(self, item: SampleDict) -> None:
        n,d = self.trainig_subset
        if self.counter % d< n:
            self._trainig.append(TrainingKnownSample(**item))
        else:
            self._testint.append(TestingKnownSample(**item))
            self.counter +=1

    @property
    def trainig(self) -> list[TestingKnownSample]:
        return self._trainig

    
    @property
    def testing(self) -> list[TestingKnownSample]:
        return self._testint
        ...
